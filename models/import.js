...
class TrelloCreator {
  constructor(data) {
    ...
    // Map of labels Trello ID => spartug ID
    this.labels = {};
    // Map of lists Trello ID => spartug ID
    this.lists = {};
    // The comments, indexed by Trello card id (to map when importing cards)
    this.comments = {};
    // the members, indexed by Trello member id => spartug user ID
    ...
    if(trelloBoard.memberships) {
      trelloBoard.memberships.forEach((trelloMembership) => {
        ...
        if(this.members[trelloId]) {
          const spartugId = this.members[trelloId];
          // do we already have it in our list?
          const spartugMember = boardToCreate.members.find((spartugMember) => spartugMember.userId === spartugId);
          if(spartugMember) {
            // we're already mapped, but maybe with lower rights
            if(!spartugMember.isAdmin) {
              spartugMember.isAdmin = this.getAdmin(trelloMembership.memberType);
            }
          } else {
  ...
  /**
   * Create the spartug cards corresponding to the supplied Trello cards,
  ...
  createCards(trelloCards, boardId) {
    ...
    trelloCards.forEach((card) => {
      ...
      // add members {
      if(card.idMembers) {
        const spartugMembers = [];
        // we can't just map, as some members may not have been mapped
        card.idMembers.forEach((trelloId) => {
          if(this.members[trelloId]) {
            const spartugId = this.members[trelloId];
            // we may map multiple Trello members to the same spartug user
            // in which case we risk adding the same user multiple times
            if(!spartugMembers.find((wId) => wId === spartugId)){
              spartugMembers.push(spartugId);
            }
          }
          return true;
        });
        if(spartugMembers.length>0) {
          cardToCreate.members = spartugMembers;
        }
      }
      // insert card
          ...
          if(Meteor.isServer) {
            file.attachData(att.url, function (error) {
              ...
              if (error) {
                ...
                const spartugAtt = Attachments.insert(file, () => {
                  // we do nothing
                });
                //
                if(trelloCoverId === att.id) {
                  Cards.direct.update(cardId, { $set: {coverId: spartugAtt._id}});
                }
              }
            });
          }
  ...
  getColor(trelloColorCode) {
    // trello color name => spartug color
    const mapColors = {
      'blue': 'belize',
      'orange': 'pumpkin',
      'green': 'nephritis',
    ...
    const spartugColor = mapColors[trelloColorCode];
    return spartugColor || Boards.simpleSchema()._schema.color.allowedValues[0];
  }
  ...
  getPermission(trelloPermissionCode) {
    ...
    // spartug does NOT have organization level, so we default both 'private' and
    // 'org' to private.
    return 'private';
  }
    ...
    trelloActions.forEach((action) => {
      if (action.type === 'addAttachmentToCard') {
        ...
        if(trelloAttachment.url) {
          // we cannot actually create the spartug attachment, because we don't yet
