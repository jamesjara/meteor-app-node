BlazeComponent.extendComponent({
  ...
  finishImport() {
    ...
    if (membersMapping) {
      ...
      membersMapping.forEach((member) => {
        if (member.spartugId) {
          mappingById[member.id] = member.spartugId;
        }
      });
  ...
  _prepareAdditionalData(dataObject) {
    ...
    // mapping is done, we add a 'spartug' field to the object representing the
    // imported member
    const membersToMap = dataObject.members;
    // auto-map based on username
    membersToMap.forEach((importedMember) => {
      const spartugUser = Users.findOne({ username: importedMember.username });
      if (spartugUser) {
        importedMember.spartugId = spartugUser._id;
      }
    });
...
BlazeComponent.extendComponent({
  onCreated() {
    this.autorun(() => {
      this.parentComponent().membersToMap.get().forEach(({ spartugId }) => {
        if (spartugId) {
          this.subscribe('user-miniprofile', spartugId);
        }
      });
    });
  },
  ...
  mapSelectedMember(spartugId) {
    return this._setPropertyForMember('spartugId', spartugId, null);
  },

  unmapMember(memberId){
    return this._setPropertyForMember('spartugId', null, memberId);
  },
